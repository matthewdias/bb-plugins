...
/**
 * The drawing behind a name, for callers that need the shape rather than the
 * component — an icon picker converting a choice into icon data, say. Extended
 * icons arrive asynchronously, so this returns null until
 * `preloadExtendedIcons()` has resolved; subscribe with
 * `subscribeExtendedIcons` to re-read once they land.
 */
export function getIconGlyph(name: string): IconSvgElement | null {
  const core = CORE_ICON_LOOKUP[name];
  if (core !== undefined) return core;
  const extended: Readonly<Record<string, IconSvgElement | undefined>> | null =
    getExtendedIcons();
  return extended?.[name] ?? null;
}
